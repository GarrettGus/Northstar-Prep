...
2. Set server-only environment variables:
   ...
   - `BLOB_READ_WRITE_TOKEN`: connect a Vercel Blob store to the project (Vercel marketplace) to set this automatically. Inventory/shopping item images are uploaded here instead of Postgres (without it, saving an image-carrying item fails closed with a 503); the daily backup cron also uses it to copy each encrypted backup off-site (without it, backups still succeed, just without an off-site copy — see [Backups and migration](#backups-and-migration)).
   ...
   - `RESEND_API_KEY` and `EMAIL_FROM` (optional): send invitations, password reset links, backup-failure notices and the opt-in weekly readiness digest by email through [Resend](https://resend.com). `EMAIL_DIGEST_WEEKDAY` (optional, default 1/Monday, UTC) picks which day the weekly digest goes out. Without these, every one of those stays exactly as it is today — a one-time link an owner copies and sends themselves, and no digest.
   - `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` and `VAPID_SUBJECT` (optional): enable web push notifications for the daily digest of overdue reminders, expiring supplies and due restocks. Generate a key pair once with `npx web-push generate-vapid-keys`; `VAPID_SUBJECT` is a `mailto:` address or URL identifying the sender, required by the push protocol. Without all three set, the "Turn on notifications" control in Household settings stays hidden.
...
5. If Postgres itself is gone rather than just corrupted (so none of the above can reach it at all), see [Recovery procedure for a total database loss](#recovery-procedure-for-a-total-database-loss).
...
Each encrypted backup is also copied to object storage (the same Vercel Blob store inventory images use), stored **private** and namespaced by household, so a single Postgres loss no longer takes every backup with it too. The off-site copy is pruned on the same 30-backup retention as the Postgres copy, and a copy failure is recorded against that backup's history (**Household settings** shows "off-site copy failed") without failing the backup itself — the Postgres copy the cron exists for is unaffected either way. If a backup's Postgres copy turns out corrupted or tampered with, restoring it transparently falls back to the off-site copy instead, running it through the exact same decrypt, checksum and schema validation before showing the same non-destructive preview. Without `BLOB_READ_WRITE_TOKEN` configured, off-site copies are skipped cleanly (recorded as "skipped", not a failure) and everything else behaves exactly as before this existed.

### Recovery procedure for a total database loss

The database rollback paths under [Rollback and recovery](#rollback-and-recovery) (the app's own backups, point-in-time recovery) all assume Postgres itself is still reachable. If it is not — the database is gone, not just corrupted — recover from the off-site copies directly, without needing `DATABASE_URL` or a running deployment at all:

1. Get `BACKUP_ENCRYPTION_KEY` from wherever you stored it when you generated it (a secret manager, password vault, or your own notes) — it is deliberately never stored alongside the backups themselves, in Postgres or in object storage, so losing it means the off-site copies are unrecoverable too. Get `BLOB_READ_WRITE_TOKEN` for the same Vercel Blob store the lost deployment used.
2. Run `npm run restore-off-site-backup -- list` to see the available backup IDs for the household (default household `1`), newest first — this only needs `BLOB_READ_WRITE_TOKEN`, since there is no Postgres left to ask which IDs exist.
3. Run `npm run restore-off-site-backup -- <backupId>` (needs `BACKUP_ENCRYPTION_KEY` too) to decrypt and validate that backup and write its household state to `restored-backup.json`.
4. Provision a fresh Neon database, run `npm run db:migrate` against it to create the schema and the first owner account, deploy the app pointed at it, and sign in as that owner.
5. Import `restored-backup.json` from **Household settings** — the same non-destructive merge preview as any other JSON import, so you can review it before confirming.
...
Everything in this section is optional and additive: without the environment variables it needs (see [Setup](#setup)), the app behaves exactly as it did before it existed.

**Email**, via [Resend](https://resend.com): invitations and owner-generated password reset links are emailed to their recipient the moment they're created, alongside the existing one-time link an owner can still copy and send themselves. A failed automatic backup emails every owner what went wrong. Any member can opt in to a weekly readiness email from **Household settings** — the same water/food/power days-of-supply and low-stock/expired counts already shown on the Dashboard, sent on `EMAIL_DIGEST_WEEKDAY` (default Monday, evaluated in UTC). An unconfigured or failed send never blocks the action it rides along with: invitations, resets and backups all succeed or fail on their own merits.

**Push**, via the Web Push protocol (VAPID): **Household settings → Notifications** offers to turn on browser push notifications once a member grants permission. The daily backup cron (see [Backups and migration](#backups-and-migration)) also sends a digest push — overdue maintenance reminders, supplies expiring within a week and items due for restock — to every subscribed browser, skipped entirely on a day with nothing to report. Tapping a notification deep-links back into the app via the service worker (`public/sw.js`). A subscription the push service reports as expired or invalid is deleted automatically rather than retried.

Both digests ride the same daily cron backups already use, rather than adding a second scheduled job — Vercel's Hobby plan allows only one. Neither can fail the backup the cron exists for: a digest failure is logged and the backup still succeeds.
