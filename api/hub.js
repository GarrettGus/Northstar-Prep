...
function summarizeAction(action, previousState) {
  ...
  switch (action.type) {
    ...
    case 'bulk_update':
      return {collection: action.collection, itemId: null, itemName: `${Array.isArray(action.ids) ? action.ids.length : 0} items`};
    case 'consume':
      return {collection: 'consumption', itemId: action.itemId, itemName: nameOf('inventory', action.itemId)};
    case 'delete_consumption': {
      const event = previousState?.consumption?.find(row => row.id === action.id);
      return {collection: 'consumption', itemId: event?.itemId ?? null, itemName: nameOf('inventory', event?.itemId)};
    }
