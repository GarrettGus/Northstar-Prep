...
export function createHandler(repository = {readState,compareAndSave}, authenticate = validSession) {
  return async (req,res) => {
    ...
    if (!authenticate(req)) return res.status(401).json({error:'Please sign in again.'});
    ...
    } catch { return res.status(503).json({error:'Database unavailable. Your changes were not confirmed; refresh before retrying.'}); }
  };
}
