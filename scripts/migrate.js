...
await sql`CREATE TABLE IF NOT EXISTS northstar_consumption_events (
  household_id integer NOT NULL REFERENCES northstar_household(id),
  id text NOT NULL,
...
await sql`CREATE INDEX IF NOT EXISTS northstar_consumption_events_item_date_idx
  ON northstar_consumption_events (household_id, item_id, event_date, created_at)`;
...
if (inventoryRows === 0 && settingsRows === 0) {
  ...
  if (existing) {
    ...
    for (const event of state.consumption) {
      await sql`INSERT INTO northstar_consumption_events (household_id, id, item_id, event_date, quantity, note)
        VALUES (1, ${event.id}, ${event.itemId}, ${event.date}, ${event.quantity}, ${event.note})
        ON CONFLICT (household_id, id) DO NOTHING`;
    }
