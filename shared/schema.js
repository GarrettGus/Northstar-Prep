...
const consumptionQuantity = z.coerce.number().finite().positive().max(1e9);
...
export function normalizeBackup(input, makeId) {
  ...
  if (!Array.isArray(consumption)) throw new Error('consumption must be an array.');
  result.consumption = consumption;
  if (new Set(consumption.map(event => event.id)).size !== consumption.length) throw new Error('Duplicate consumption event IDs in backup.');
...
export function applyAction(state, action) {
  ...
  if (action.type === 'import') {
    ...
    const mergedConsumption = new Map(next.consumption.map(event => [event.id, event]));
    backup.consumption.forEach(event => mergedConsumption.set(event.id, event));
    next.consumption = [...mergedConsumption.values()];
    ...
    const itemId = idSchema.parse(action.itemId);
    if (!next.inventory.some(item => item.id === itemId)) throw new Error('The inventory item no longer exists. Refresh and retry.');
    const event = consumptionEventSchema.parse({
      id: action.id,
      itemId,
      date: action.event?.date ?? action.date,
      quantity: action.event?.quantity ?? action.quantity,
      note: action.event?.note ?? action.note,
    });
    if (next.consumption.some(existing => existing.id === event.id)) throw new Error('Consumption event already exists.');
    next.consumption.push(event);
  } else if (action.type === 'delete_consumption') {
    const id = idSchema.parse(action.id);
    next.consumption = next.consumption.filter(event => event.id !== id);
