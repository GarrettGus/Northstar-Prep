...
test('consumption events validate, merge by ID and leave current stock unchanged',()=>{
  const stocked=applyAction(emptyState(),{type:'add',collection:'inventory',id:'rice',item:{name:'Rice',category:'Food',quantity:10,unit:'bags'}});
  const first=applyAction(stocked,{type:'consume',id:'use-1',itemId:'rice',event:{date:'2026-09-10',quantity:2,note:'Dinner'}});
  assert.equal(first.inventory[0].quantity,10);
  assert.equal(first.consumption.length,1);
  assert.throws(()=>applyAction(stocked,{type:'consume',id:'use-2',itemId:'missing',date:'2026-09-10',quantity:1}));
  assert.throws(()=>applyAction(stocked,{type:'consume',id:'use-2',itemId:'rice',date:'2026-09-10',quantity:0}));
  const backup=normalizeBackup({consumption:[{id:'use-2',itemId:'rice',date:'2026-09-11',quantity:1}]},()=> 'unused');
  const merged=applyAction(first,{type:'import',backup});
  assert.deepEqual(merged.consumption.map(event=>event.id),['use-1','use-2']);
  assert.deepEqual(applyAction(merged,{type:'import',backup}),merged);
});
test('burn rate and depletion forecasts require dated history and use local calendar dates',()=>{
  const item={id:'rice',quantity:10,unit:'bags'};
  const events=[
    {id:'use-1',itemId:'rice',date:'2026-09-10',quantity:2,note:''},
    {id:'use-2',itemId:'rice',date:'2026-09-12',quantity:4,note:''},
  ];
  assert.equal(deriveBurnRate([events[0]]),null);
  assert.equal(deriveBurnRate([{...events[0],date:'2026-09-12'},events[1]]),null);
  const rate=deriveBurnRate(events);
  assert.equal(rate.ratePerDay,3);
  assert.equal(rate.daysOfHistory,2);
  const forecast=consumptionForecast(item,events,new Date(2026,8,12,19,30));
